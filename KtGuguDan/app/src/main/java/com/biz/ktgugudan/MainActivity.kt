...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Subject중에서 가장 많이 사용하는 구현체
        // 최근 마지막까지 캐시를 만들어서 구현하는 클래스
        // 아무런 메시지가 없을때 문자열 "0"으로 토기화하여 객체를 생성하라
        ...
        txt.addTextChangedListener(object : TextWatcher{
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                // app, device 입장에서는 산당히 많은 비용이 소요되는 코드
                // view.text = text
                subject.map { txt.text.toString() == ""} // 최초의 app이 실행되거나,
                                                        // EditText에 문자열이 모두 제거되면
                    //java Stream(연속된 데이터)형태의 자료를 생성하는 method
                    // flatMap에 의해 생성된 데이터는 Oservable형태의 데이터이다
                    // Observable관련된 method 등을 chain하여 데이터를 처리할 수 있다.
                 ...
                 * 그럼에도 불구하고 subejct(rxjava) 코드를 사용하는 이유는
                ...
                subject.map { txt.text.toString().toLong() }
                    ...
                    .scan { x, y -> x + y }
                    .subscribe ({ text -> view.text = text })
                    {obj : kotlin.Throwable -> obj.message }
            }
