...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fab.setOnClickListener { view ->
            ...
            // 어떤 값을 return하면 그 값을 수신하겠다 라는 전제하에 사용하는 method
    ...
    // onCreate()에서 setSupportActionBar 메서드가 실행될떄 호출
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
         ...
         * Activity를 추가한다.
         * 그러면 AndroidManifext.xml에 Activity가 등록된다.
         * 
        ...
        return when (item.itemId) {
            ...
            R.id.action_login -> { // 햄버거 / 로그인메뉴 선택시
                ...
                loginIntent.putExtra("username", "nong@nong.com")
                startActivity(loginIntent)
                true
            }
            R.id.action_phone -> {
                val number : String = "0102226974"
                 ...
                 * 전화걸기 어플이 무었이 있던지 상관없이 내가 만든 어플에서
                 * 전화걸기를 수행할 수 있다.
                 * 특정한 어플에 종속되는 것을 막아서 어플이 오류가 나는 것을 최소화하기 위한 것
                 */
                val phoneDialIntent = Intent(Intent.ACTION_DIAL, Uri.parse(number))
                val phoneCllButtonIntent = Intent(Intent.ACTION_CALL_BUTTON, Uri.parse(number))
            ...
            R.id.action_internet ->{
                ...
                // 인터넷의 특정한 주소 페이지를 열고자 할때
                val naver = "https://naver.com"
                
                // Uri 부분을 생략하면 기본 browser가 열림
                val internetIntent = Intent(Intent.ACTION_VIEW, parse(naver))
